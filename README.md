...
Currently, That isn't a thing at the moment, In the tracker's current state, AP will not be able to tell the tracker what entrance/exit the user went through on it's own without user input. 
This means that you will have to manually check off entracnes/exits if you were playing WWR with Shuffled Entrances turned on.
